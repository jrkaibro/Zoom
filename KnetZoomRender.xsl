...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'KnetZoom'">
        <xsl:call-template name="RenderKnetZoom"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- KnetZoom design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderKnetZoom">
    <span atomicselection="true">
      <xsl:call-template name="AddStyleAttribute"/>
      
      
    <div>      
	   <xsl:attribute name ="id">KnetZoom</xsl:attribute>
   	  <img>
